...
**Reason**: Jot is becoming a web-primary, structured tool; the LLM is reduced to a stateless command surface and no longer needs to carry conversational state.
**Migration**: The `conversations` table is dropped on migration for existing DBs. Anything worth retaining must already live in structured tables (`things`, `schedules`); there is no automated export of prior conversation text.

### Requirement: Conversation auto-summarization

**Reason**: With no persistent history, summaries have no consumer; the >10 min gap-detection trigger and summary pruning are dead weight.
**Migration**: The `conversation_summaries` table is dropped on migration. No replacement; summaries are not regenerated.

### Requirement: Free-text memories

**Reason**: Remembering free-text preferences/observations is no longer a goal for Jot; everything worth remembering will live in structured tables.
**Migration**: The `memories` table, its `memories_fts` index, and sync triggers are dropped on migration. The five memory LLM tools are removed. Existing memory rows are not migrated into `things`; users who want a memory preserved should re-capture it as a thing.
